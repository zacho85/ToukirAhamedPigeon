...
export default function TontineCard({ tontine }: { tontine: any }) {
    ...
    <Card className="rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex -space-x-2 overflow-hidden pt-2">
          {tontine.members?.slice(0, 5).map((member: any, i: number) => (
            <img
              key={i}
              className="inline-block h-8 w-8 rounded-full ring-2 ring-white"
              src={`https://avatar.vercel.sh/${member.user_id}.png?text=${member.user_id.charAt(0)}`}
              alt={`Member ${i}`}
            />
          ))}
