...
export class AuthService {
  ...
  private generateQrToken(userId: number) {
    return `USER_${userId}_${Date.now()}_${Math.random()
      .toString(36)
      .substring(2, 10)}`;
  }

  private async generateQrImage(qrToken: string) {
  ...
  async register(userData: any) {
    try {
      ...
      const qrToken = this.generateQrToken(userCount + 1);
      const qrCode = await this.generateQrImage(qrToken);
  ...
  // Initiate password reset (revokes previous links)
    ...
    async getUserQRCode(userId: number) {
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { qrCode: true },
    });
  ...
  async regenerateUserQRCode(userId: number) {
    const user = await this.prisma.user.findUnique({ where: { id: userId } });
    if (!user) throw new NotFoundException('User not found');

    const qrToken = this.generateQrToken(user.id);
    const qrCode = await this.generateQrImage(qrToken);
  ...
  async getUserByQRCode(qrToken: string) {
    if (!qrToken) {
